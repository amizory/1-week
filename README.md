...
```txt
Какие Linux-дистрибутивы лучше подходят для Docker-образов? Почему?
Как добавить новый репозиторий в пакетный менеджер?
Как можно выполнить одну команду, только если предыдущая выполнилась успешно?
Как можно выполнить одну команду, только если предыдущая выполнилась неуспешно?
Какие есть типы ссылок в Linux? Чем они отличаются? 
Как создать ссылку на файл/директорию?
Как создать нового пользователя в Linux из скрипта (non interactive mode)?
Как поменять пароль пользователя?
Что означает execute бит на директории? 
Какие минимальные права должны быть у директории и файла в ней чтобы пользователь-владелец мог его прочитать?"
```

### Advanced-5

```txt
Как осуществить процесс установки Linux на группу серверов в корпоративной сети?
Как зафиксировать нужную версию пакета в системы, чтобы он не обновлялся автоматически?
Как можно самому собрать .deb или .rpm пакет?
Как вывести в консоль размер всех файлов в текущей директории в отсортированном по убыванию виде?
За что отвечают файлы /etc/sudoers, /etc/passwd, /etc/shadow ?
Как разрешить пользователю выполнять все команды на хосте без ввода пароля?
Что такое sticky bit, suid, sgid? Где и зачем они применяются?
```
